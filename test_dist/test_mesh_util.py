...
class MeshUtilTest(unittest.TestCase):
  ...
  def test_init_module_sharded(self):
    ...
    self.assertEqual(len(model.a.weight.jax().addressable_shards), len(jax.devices()))
    self.assertEqual(len(model.a.bias.jax().addressable_shards), len(jax.devices()))
  ...
  def test_sharder_call_no_shardable(self):
    ...
    with self.assertRaisesRegex(AssertionError, "Unable to find a dim to shard"):
