...
class Sampler(nn.Module):
  ...
  def forward(
    ...
    next_token_ids = torch.multinomial(probs, num_samples=1, replacement=True).squeeze(
      dim=-1
    )
...
class GemmaAttention(nn.Module):
  def __init__(
    ...
    self.o_proj = Linear(self.num_heads * self.head_dim, self.hidden_size, quant=quant)
...
class GemmaForCausalLM(nn.Module):
  ...
  def generate(
    ...
    for _i in range(max_seq_len - min_prompt_len):
      next_token_ids = self(
