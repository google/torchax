...
def diff_output(
  testcase, output1, output2, rtol, atol, equal_nan=True, check_output=True
):
  if isinstance(output1, torch.Tensor):
    ...
    if check_output:
      ...
      testcase.assertEqual(
        (output1.shape, output1.dtype), (output2.shape, output2.dtype)
      )
...
def run_export_and_compare(
  testcase, func, sample_input, check_output=True, equal_nan=True, ignore_indices=False
):
  atol, rtol = (1e-3, 1e-5)
...
ops_to_test = [
  ...
  for test in op_db
  if (
    test.name not in (skiplist | not_support_ops_list)
    and test.variant_test_name not in variant_test_name_to_skip
  )
]
  ...
  def test_reference_eager(self, device, dtype, op):
    ...
    for sample_input in sample_inputs:
      ...
      if op.name == "polygamma" or op.name == "special.polygamma":
        ...
        sample_input.input = self.replace_values_below_threshold(sample_input.input, 1)
